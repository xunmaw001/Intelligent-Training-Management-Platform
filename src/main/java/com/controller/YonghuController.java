...
public class YonghuController {
     ...
     * 充值会员
     */
    @GetMapping(value = "/congzhihuiyuan")
    public R congzhihuiyuan(Integer  id,String huiyuandaoqiTime,Double newMoney){
        if(newMoney < 1000){
            return R.error("余额不足");
        }
        ...
        try {
            if(huiyuandaoqiTime != null){
                cd.setTime(sdf.parse(huiyuandaoqiTime));
            }else{
                cd.setTime(new Date());
            }
        ...
        yonghu.setNewMoney(newMoney - 1000);
        yonghu.setHuiyuandaoqiTime(cd.getTime());
        yonghu.setId(id);
        yonghu.setHuanyuanTypes(2);
        yonghuService.updateById(yonghu);

        XiaofeijiluEntity xiaofeijiluEntity = new XiaofeijiluEntity();
        xiaofeijiluEntity.setCreateTime(new Date());
        xiaofeijiluEntity.setInsertTime(new Date());
        xiaofeijiluEntity.setYonghuId(id);
        xiaofeijiluEntity.setXiaofeijiluQian(1000D);
        xiaofeijiluEntity.setXiaofeijiluName("购买或续费会员");
        xiaofeijiluService.insert(xiaofeijiluEntity);
        return R.ok();
    }
...
//                            yonghuEntity.setHuanyuanTypes(Integer.valueOf(data.get(0)));   //会员类型 要改的
//                            yonghuEntity.setHuiyuandaoqiTime(sdf.parse(data.get(0)));          //会员到期时间 要改的
