...
//                            xiaofeijiluEntity.setXiaofeijiluName(data.get(0));                    //消费项目 要改的
//                            xiaofeijiluEntity.setXiaofeijiluQian(data.get(0));                    //消费金额 要改的
...
//                            xiaofeijiluEntity.setCreateTime(date);//时间
    ...
    */
    @RequestMapping("/add")
    public R add(@RequestBody XiaofeijiluEntity xiaofeijilu, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,xiaofeijilu:{}",this.getClass().getName(),xiaofeijilu.toString());
        Wrapper<XiaofeijiluEntity> queryWrapper = new EntityWrapper<XiaofeijiluEntity>()
            .eq("yonghu_id", xiaofeijilu.getYonghuId())
            .eq("xiaofeijilu_name", xiaofeijilu.getXiaofeijiluName())
            ;
